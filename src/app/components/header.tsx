...
//               <Image src="/assets/logo.svg" alt="Evernest Logo" width={120} height={32} className="h-8 w-auto object-contain" priority
//               />
//             </Link>
...
//               </button>
//             </div>
//             <button className="lg:hidden p-3 relative z-50" onClick={() => setIsMobileMenuOpen((v) => !v)} aria-label="Toggle menu">
...
export default function Header() {
  ...
  const createLocalizedUrl = (url: string) => {
    ...
    // If already contains locale, return as is
    if (cleanUrl.startsWith(`/${locale}`)) return cleanUrl;

    const segments = cleanUrl.split("/").filter(Boolean);
    const validLocales = ["en", "cz", "ru", "es"];
    if (segments.length > 0 && validLocales.includes(segments[0])) return cleanUrl;
      ...
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
          <nav className="flex items-center justify-between h-20 sm:h-24">
            {/* Logo */}
            <Link
              href={createLocalizedUrl(createPageUrl("Home"))}
            ...
            {/* ✅ Desktop Menu (show only on XL and above) */}
            <div className="hidden xl:flex items-center">
              {navItems.map((item) => {
                const itemUrl = createLocalizedUrl(item.url);
                  ...
                  >
                    <Link
                        ...
                        className={`
                          ...
                          px-3 xl:px-4 py-1
                          whitespace-nowrap
                          text-[15px] md:text-[16px] xl:text-[19px]
                          ...
                          after:bg-[color:var(--brand)] after:transition-all after:duration-300
                          after:-translate-x-1/2 hover:after:w-4/5
                          ${isActive ? "text-[color:var(--brand)] after:w-3/5" : ""}
                        `}
                      >
                        {item.label}
                      </Link>                     
                        ...
                        {activeDropdown === item.label && (
                          ...
                          >
                            {item.dropdown.map((dropItem, index) => (
                              <Link
                                key={index}
                                href={createLocalizedUrl(createPageUrl(dropItem.routeKey))}
                                className="block px-3 py-2 text-[15px] xl:text-[17px] text-[#1a1a1a]/75 hover:text-[color:var(--brand)] hover:bg-[color:var(--cream)] transition-all duration-200 border-b border-black/5 last:border-0 font-light"
                                onClick={() => setActiveDropdown(null)}
                              >
                                {dropItem.label}
                              </Link>
            ...
            <div className="hidden xl:flex items-center gap-4">
              ...
              >
                ...
                >
                  <span
                    className={`text-base ${
                      languages.find((l) => l.code === currentLang)?.flagClass
                    }`}
                  ></span>
                  <span>{currentLang.toUpperCase()}</span>
                  <svg
                ...
                <AnimatePresence>
                  {langOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -6 }}
                      transition={{ duration: 0.15 }}
                      className="absolute right-0 mt-2 w-36 bg-white border border-black/10 shadow-xl rounded-sm overflow-hidden z-50"
                    >
                      {languages.map((lang) => (
                        <button
                          key={lang.code}
                        ...
                        >
                          <span className={`text-base ${lang.flagClass}`}></span>
                          <span className="flex-1 text-left text-black/60">
                            {lang.name}
                          </span>
                          <span className="text-xs text-black/60">{lang.label}</span>
                        </button>
              ...
              <button
                onClick={() => setIsContactOpen(true)}
                className="cursor-pointer relative px-6 py-2 text-sm font-normal tracking-wide rounded-sm text-white bg-gradient-to-r from-[color:var(--brand)] to-[#c97a52] overflow-hidden transition-all duration-300"
              >
                <span className="relative z-10">{t("bookNow")}</span>
                <span
                  className="absolute inset-0 w-[180%] h-[180%] -top-[40%] -left-[40%] rotate-45 bg-gradient-to-r from-transparent via-white/70 to-transparent blur-[1px] animate-[luxury-shine_1.6s_ease-in-out_infinite] pointer-events-none"
                ></span>
              </button>
            </div>

            {/* ✅ Mobile Hamburger (for < XL) */}
            <button
              className="xl:hidden p-3 relative z-50"
              onClick={() => setIsMobileMenuOpen((v) => !v)}
              aria-label="Toggle menu"
            >
              <div className="relative w-6 h-5">
                <span
                  className={`absolute top-0 left-0 w-6 h-0.5 bg-black transition-all duration-300 ${
                    isMobileMenuOpen ? "rotate-45 top-2" : ""
                  }`}
                />
                <span
                  className={`absolute top-2 left-0 w-6 h-0.5 bg-black transition-all duration-300 ${
                    isMobileMenuOpen ? "opacity-0" : ""
                  }`}
                />
                <span
                  className={`absolute top-4 left-0 w-6 h-0.5 bg-black transition-all duration-300 ${
                    isMobileMenuOpen ? "-rotate-45 top-2" : ""
                  }`}
                />
              </div>
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              animate={{ opacity: 1, height: "auto" }}
              exit={{ opacity: 0, height: 0 }}
              className="xl:hidden bg-white border-t border-black/10"
            >
              <div className="container mx-auto px-4 sm:px-6 py-6 space-y-2">
                    ...
                    {languages.map((lang) => (
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded text-sm transition-all ${
                          currentLang === lang.code
                            ? "bg-[color:var(--cream)] text-[color:var(--brand)] border border-[color:var(--brand)]/30"
                            : "hover:bg-[color:var(--cream)] border border-black/10"
                        }`}
                      >
                        <span className={`${lang.flagClass}`}></span>
                        <span>{lang.name}</span>
                ...
                {/* Mobile Nav Links (includes dropdown items too) */}
                {navItems.map((item) => (
                  <div key={item.label} className="space-y-1">
                    ...
                    {item.dropdown?.map((dropItem, index) => (
                      <Link
                        key={index}
                        href={createLocalizedUrl(createPageUrl(dropItem.routeKey))}
                        className="block ml-6 px-4 py-2 text-sm text-[#1a1a1a]/55 hover:text-[color:var(--brand)] hover:bg-[color:var(--cream)] rounded-lg transition-all duration-200"
                        onClick={() => setIsMobileMenuOpen(false)}
                      >
                        • {dropItem.label}
                      </Link>
