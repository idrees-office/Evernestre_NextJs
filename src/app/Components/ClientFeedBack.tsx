...
//             Real <span className="text-[#c2754e]">testimonials</span> from our valued clients
//           </p>
//         </div>
...
//               <button
//                 onClick={prevSlide}
//                 className="absolute -left-3 sm:-left-5 top-1/2 -translate-y-1/2 z-10 w-9 h-9 bg-white rounded-full shadow-md flex items-center justify-center hover:shadow-lg transition-shadow border border-gray-100"
//                 aria-label="Previous"
//               >
...
//               <button
//                 onClick={nextSlide}
//                 className="absolute -right-3 sm:-right-5 top-1/2 -translate-y-1/2 z-10 w-9 h-9 bg-white rounded-full shadow-md flex items-center justify-center hover:shadow-lg transition-shadow border border-gray-100"
//                 aria-label="Next"
//               >
...
export default function TestimonialSlider() {
  const [testimonials, setTestimonials] = useState<Testimonial[]>([]);
  const [totalReviews, setTotalReviews] = useState<number>(327); // Default value from image
  const [currentSlide, setCurrentSlide] = useState(0);
  const [slidesToShow, setSlidesToShow] = useState(3);
  const [loading, setLoading] = useState(true);
    ...
    async function loadReviews() {
      try {
        ...
        if (data.status === 'success') {
          ...
          // Use total_reviews from API if available, otherwise keep default
    ...
    <section className="bg-[#faf8f5] py-10 px-4 sm:px-6 lg:px-10">
      <div className="max-w-8xl mx-auto">
        ...
        {testimonials.length > 0 ? (
          <div className="relative">           
            {testimonials.length > slidesToShow && (
              <>
                <button
                  onClick={prevSlide}
                  className="absolute -left-3 sm:-left-5 top-1/2 -translate-y-1/2 z-10 w-9 h-9 bg-white rounded-full shadow-md flex items-center justify-center hover:shadow-lg transition-shadow border border-gray-100 cursor-pointer"
                  aria-label="Previous"
                >
                  <ChevronLeft className="w-5 h-5 text-[#c2754e]" />
                </button>
                <button
                  onClick={nextSlide}
                  className="absolute -right-3 sm:-right-5 top-1/2 -translate-y-1/2 z-10 w-9 h-9 bg-white rounded-full shadow-md flex items-center justify-center hover:shadow-lg transition-shadow border border-gray-100 cursor-pointer"
                  aria-label="Next"
                >
                  <ChevronRight className="w-5 h-5 text-[#c2754e]" />
                </button>
          ...
          // No reviews message (if API returns empty but we have the badge)
          <div className="text-center py-8 bg-white rounded-lg border border-gray-200">
            <p className="text-gray-500">No recent testimonials available.</p>
            <p className="text-sm text-gray-400 mt-1">Check back later for new reviews.</p>
          </div>
        )}
      </div>
    </section>
